...
my $Putative_SNP     = "$outdir/smcandidatesnps_D15d0.txt";
...
print "$fq1_left,$fq1_right-$fq2_left,$fq2_right\n";
...
sub command_tophat{
        ...
        print "bowtie2 -k 1  --no-unal --omit-sec-seq -p $cpu -x $bowtie_index -1 $s_left -2 $s_right -S $outdir/acc.sam";
        !system "bowtie2 -k 1  --no-unal --omit-sec-seq -p $cpu -x $bowtie_index -1 $s_left -2 $s_right -S $outdir/acc.sam 2> $outdir/compare.log" or die "Error in bowtie2:$!";
